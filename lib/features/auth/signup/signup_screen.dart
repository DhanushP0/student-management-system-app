    ...
    try {
      ...
      if (userId != null) {
        final studentData = {
          ...
          'uucms_id': uucmsId,
          'role_id':
              '1392a59a-1ddc-4bf5-a5a2-20e7a177ad7c', // Hardcoded Student role ID
        };
        ...
        try {
          ...
          print('Inserted student: $inserted');
        } catch (error) {
          // This catches any error thrown by the insert operation
        ...
        if (mounted) {
          ...
          // Optionally navigate to another screen, e.g., the dashboard
          context.go('/');
        }
      }
    } catch (e) {
                  ...
                  Form(
                    ...
                    child: Column(
                      children: [
                        ...
                        // UUCMS ID - Always Displayed
                        _buildCupertinoFormField(
                          controller: _uucmsController,
                          placeholder: "UUCMS ID",
                          icon: CupertinoIcons.number,
                          validator: (val) {
                            // Ensure the UUCMS ID follows the format (e.g., U18EV22S0174)
                            final regex = RegExp(
                              r"^U\d{2}[A-Z]{2}\d{2}[S]\d{4}$",
                            );
                            if (val != null && !regex.hasMatch(val)) {
                              return 'Please enter a valid UUCMS ID (e.g., U18EV22S0174)';
                            }
                            return null;
                          },
                        ),
                        ...
                        // Temporary Teacher Signup Button
                        Center(
                          child: CupertinoButton(
                            padding: EdgeInsets.zero,
                            onPressed: () => context.go('/teacher-signup'),
                            child: const Text(
